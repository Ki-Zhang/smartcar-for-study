...
void EXTI0_IRQHandler(void)
{
	...
	{
		idx = 0;
		g_image = mt9v03x_image[idx];
	}
	else if (mt9v03x_finish_flag[1])
	{
		idx = 1;
		g_image = mt9v03x_image[idx];
	}
	...
	mt9v03x_busy_flag[idx] = 1;
	mt9v03x_finish_flag[idx] = 0;
...
void DMA1_Channel4_IRQHandler(void)
{
	if(SET == DMA_GetFlagStatus(DMA1_FLAG_TC4))										// 判断触发通道
	{
		...
		switch(camera_type)															// 查询摄像头类型 未初始化摄像头则此处会进入default
		{
			...
			case CAMERA_GRAYSCALE: 													// 总钻风
				mt9v03x_dma();
				EXTI_GenerateSWInterrupt(EXTI_IMR_0);
				break;
			default:
				break;
		}
